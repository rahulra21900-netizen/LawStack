...
// Badge Component
export function Badge({ label, variant = "neutral", className }: { label: string; variant?: "success" | "warning" | "error" | "info" | "neutral"; className?: string }) {
  const styles = {
  ...
  return (
    <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider", styles[variant], className)}>
      {label}
    </span>
  );
}
